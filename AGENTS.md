...
- The cleanup commits were pushed to `origin/my-new-branch` (GitHub) on 2026-09-24. The push to `overleaf` was initially rejected because a co-author had made prose edits directly in the Overleaf web editor (`93d63f5 Update on Overleaf.`) after the last sync — these touched `paper.tex` (Introduction/Study Setting wording, minor paragraph breaks) and `references.bib` (two new citations: `mcfarland2014network-735`, `hachen2022generators-923`), but nothing else. Since the cleanup commits never touched `paper.tex`/`references.bib`, `git merge overleaf/main` resolved with no conflicts. The merge commit (`6e54413`) has been pushed to `origin`; **still needs to be pushed to `overleaf` (`git push overleaf my-new-branch:main`) to close the loop** — do this only after explicit user confirmation, and recompile with `latexmk -pdf paper.tex` afterward to confirm the merged prose still compiles.
- **Lesson for future sessions:** always `git fetch overleaf && git diff HEAD overleaf/main` before pushing to `overleaf`, even if `git status` looks clean locally — edits can land there via the web UI with no local trace.
