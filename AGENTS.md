...
- The cleanup commits were pushed to `origin/my-new-branch` (GitHub) on 2026-09-24. The push to `overleaf` was initially rejected because a co-author had made prose edits directly in the Overleaf web editor (`93d63f5 Update on Overleaf.`) after the last sync — these touched `paper.tex` (Introduction/Study Setting wording, minor paragraph breaks) and `references.bib` (two new citations: `mcfarland2014network-735`, `hachen2022generators-923`), but nothing else. A second round happened the same day: after merging and pushing `6e54413` to `origin`, a re-fetch of `overleaf` turned up yet another web-editor commit (`6aeaab2`, more Wave 3/pooled-regression wording tweaks in `paper.tex`, e.g. an added MDH-consistency sentence). Both rounds merged with **zero conflicts** since the cleanup commits never touched `paper.tex`/`references.bib`. `paper.pdf` was regenerated via `latexmk -pdf paper.tex` (20 pages, no errors) after each merge and committed. **As of 2026-09-24, `origin/my-new-branch`, `overleaf/main`, and local `my-new-branch` are all fully in sync at commit `4feda41`.**
- **Lesson for future sessions:** always `git fetch overleaf && git diff HEAD overleaf/main` before pushing to `overleaf`, even if `git status` looks clean locally — edits can land there via the web UI with no local trace, and can arrive in more than one round within the same session. Re-fetch and re-diff immediately before every push, not just once at session start.
