...
As a complementary check on the same question, we also computed Wald tests of pairwise equality directly on the pooled log-odds coefficients (using the same ego-clustered variance structure as the pooled regression tables), rather than on the baseline-adjusted probability-scale excess rates used above. We test all six pairwise contrasts among the four groups, Holm-corrected for multiple comparisons (Table~\ref{tab:bootstrap-sensitivity}, Panels C--D): the three Catholic-versus-non-Catholic contrasts, included as a sanity check on the distinction the rest of this analysis treats as reliable, and the three non-Catholic-versus-non-Catholic contrasts of substantive interest. On the baseline Model 1 specification (Panel C), every Catholic-versus-non-Catholic contrast is significant after correction (all Holm-adjusted $p < 0.01$), confirming that sanity check, while none of the three non-Catholic pairwise contrasts survives correction (all Holm-adjusted $p > 0.13$; Other Religion-versus-Protestant is marginal before correction, $p = 0.044$, but not after). Re-running the identical six contrasts on the fully-adjusted Model 3 specification (Panel D, structural foci, demographic homophily, salience, and degree controls) leaves this conclusion unchanged---no non-Catholic pairwise contrast survives Holm correction (all Holm-adjusted $p > 0.08$)---though the No Religion-versus-Other Religion and Other Religion-versus-Protestant contrasts both tighten noticeably (raw $p = 0.033$ and $p = 0.027$, respectively) once these covariates are added, suggesting Other Religion's apparent separation from the other two minority groups is somewhat more pronounced net of structural and demographic controls, even though it remains short of significance after correcting for the three comparisons. This log-odds-scale test agrees with the probability-scale bootstrap comparisons above.

Together, these three independent diagnostics---testing each group's Yule's $Q$ against its own permutation null, testing pairwise differences in bootstrap-resampled excess tie rates, and testing pairwise equality of the pooled log-odds coefficients directly---converge on the same conclusion: the data reliably support treating Catholic and non-Catholic as distinct categories in this population, but they do not reliably support a finer ranking among Protestant, No Religion, and Other Religion students.
