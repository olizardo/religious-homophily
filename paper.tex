...
As a complementary check on the same question, we also computed Wald tests of pairwise equality directly on the pooled Model 1 log-odds coefficients (using the same ego-clustered variance structure as the pooled regression tables), rather than on the baseline-adjusted probability-scale excess rates used above (Table~\ref{tab:bootstrap-sensitivity}, Panel C). None of the three pairwise contrasts among Protestant, No Religion, and Other Religion is significant after a Holm correction for multiple comparisons (all Holm-adjusted $p > 0.13$); the Other Religion-versus-Protestant contrast is marginal before correction ($p = 0.044$) but not after. This log-odds-scale test agrees with the probability-scale bootstrap comparisons above.
