...
We fit four nested dyadic logistic regression models to Wave 3 student nominations (sophomore-year baseline), adjusting for tie-level covariates including physical propinquity (e.g., residing in the same dorm), gender and race homophily, individual-level religious salience, and (Model 4) each ego's activity level and each alter's popularity in the nomination network. Model 1 finds all three non-Catholic groups positive and significant relative to the Catholic majority, whose own baseline coefficient is not reliably distinguishable from zero. As Table~\ref{tab:wave3-reg} shows, once tie-level structural controls (roommates, dormitory co-residence, gender homophily, race homophily) are added in Model 2, the Protestant ($\beta = 0.600, p < 0.01$) and Other Religion ($\beta = 2.076, p < 0.001$) coefficients remain essentially unchanged, but the No Religion coefficient drops sharply and is no longer statistically distinguishable from the Catholic baseline ($\beta = 0.164, p = 0.54$); this pattern persists after adjusting for religious salience in Model 3. A reviewer raised the concern that opportunity-adjusted homophily coefficients could partly reflect activity or popularity differences by religious group rather than affiliation itself---for instance, if members of a group simply nominate, or are nominated by, more peers, an apparent tendency toward in-group ties could be a degree artifact rather than a preference. Model 4 addresses this directly by adding each ego's log out-degree (nominations made in Wave 3) and each alter's log in-degree (nominations received), both computed from the full Wave 3 nomination roster prior to any filtering on religion. Adding these degree terms leaves the Protestant ($\beta = 0.625, p < 0.01$) and Other Religion ($\beta = 2.087, p < 0.001$) coefficients essentially unchanged, and the already-null No Religion coefficient remains null ($\beta = 0.128, p = 0.64$); ego activity and alter popularity are themselves only marginally associated with same-religion tie formation ($p \approx 0.10$--$0.16$) and do not confound the group-level homophily estimates.

We caution against over-interpreting the relative magnitude of these three coefficients as evidence of a fine-grained ranking among non-Catholic subgroups. The Other Religion group comprises only 16 egos and 55 dyads at Wave 3, and a bootstrap resampling analysis at the ego level (resampling egos with replacement within each religious group, holding the wave-specific opportunity pool fixed) shows that its excess in-group tie rate is not statistically distinguishable from that of Protestant or No Religion students---the pairwise 95\% confidence intervals for all three minority-minority contrasts include zero. The large point estimate for Other Religion on the logit scale is consistent with its small denominator rather than demonstrable evidence of a comparatively stronger preference for in-group ties. We therefore treat these results as evidence that non-Catholic religious affiliation, considered collectively, is associated with elevated opportunity-adjusted homophily relative to the Catholic majority, without treating the specific ordering of coefficients across non-Catholic subgroups as a reliable finding. This result is still consistent with the MDH, however. 

A natural interpretation of the Model 1-to-Model 2 shift is that physical or demographic foci mediate homophily for the group whose coefficient changes most---here, No Religion. We do not pursue a formal mediation analysis to adjudicate this, however, since the No Religion coefficient is already statistically indistinguishable from the Catholic baseline once structural controls are added (Models 2--4); with no reliable total effect left to decompose, an ACME estimate would carry little information. In any case, this interpretation is difficult to establish from the nested coefficients alone: in logistic regression, adding covariates rescales the error-term variance, so a change in a coefficient's magnitude across nested nonlinear models cannot, by itself, distinguish genuine mediation from a rescaling artifact (or from the differential sample selection that listwise deletion introduces---the No Religion analytic sample shrinks from 35 to 25 egos between Models 1 and 2, disproportionately more than the other groups, largely because race homophily is undefined for the substantial share of No Religion egos who identify their race as ``Foreign Student'' or ``Other'').

\subsection{Pooled Longitudinal Regressions with Robust Clustered Standard Errors}

To trace how these associational boundaries evolve longitudinally throughout the college career, we fit a nested series of pooled models and an interaction model across Waves 3--8 (Table \ref{tab:pooled-interaction-reg}). Because individual students are observed repeatedly, we estimate robust standard errors clustered at the respondent level. Unlike the Wave 3 cross-section, the pooled data show the No Religion group retaining a statistically significant positive effect on active homophily throughout every specification, including after tie-level structural controls ($\beta = 0.577, p < 0.01$, Model 2) and after additionally adjusting for ego activity and alter popularity ($\beta = 0.570, p < 0.01$, Model 3); this greater stability likely reflects the much larger pooled No Religion sample (37 egos, 676 ties) compared with the single-wave cross-section. Furthermore, we find a counter-intuitive negative effect of same-dorm co-residence on the probability of forming a same-religion tie ($\beta = -0.252, p < 0.05$, Model 3), and race homophily (the dyadic tendency to bond with same-race peers) is also a significant predictor of tie formation ($\beta = 0.259, p < 0.05$, Model 3), indicating that religious and racial boundaries operate as complementary, rather than competing, axes of social sorting. As in the Wave 3 models, adding each ego's log out-degree and each alter's log in-degree (Model 3) leaves the three group coefficients essentially unchanged, again indicating that opportunity-adjusted homophily is not an artifact of differential nomination activity or popularity by religious group (Reviewer 1); the degree terms themselves are only marginally significant ($p \approx 0.07$).

The results also show that \textit{non-Catholic groups collectively maintain an elevated, opportunity-adjusted homophily relative to the Catholic majority that does not change appreciably over time}, as evidenced by broadly parallel trends across waves. As with the Wave 3 results, we caution against reading the specific magnitudes across Protestant, No Religion, and Other Religion as a stable ranking. A pooled bootstrap analysis across Waves 3--8 (resampling the 473 unique egos who contribute ties, respecting within-ego clustering across waves) confirms that Protestant and No Religion students each show a modest but statistically robust excess in-group tie rate relative to Catholics (approximately 6--10 percentage points above the chance baseline), but Other Religion---limited to only 22 unique individuals across the entire six-wave panel---remains statistically indistinguishable from every other group, including the Catholic majority. This indistinguishability persists after pooling six waves of data because the binding constraint is the number of distinct people in the group, not the number of observed ties: additional waves largely add repeated observations from the same small set of individuals rather than new independent information.
...
The evidence for the No Religion group is more mixed than for Protestant and Other Religion students. In the single-wave Wave 3 cross-section, its opportunity-adjusted coefficient is no longer statistically distinguishable from the Catholic baseline once tie-level structural controls are added ($\beta = 0.164$ to $0.128$, all $p > 0.5$, Models 2--4), and we cannot distinguish whether this reflects genuine confounding by physical or demographic foci, a logit rescaling artifact, or reduced power from differential sample attrition (see Results). In the pooled longitudinal model, by contrast, the No Religion coefficient remains positive and statistically significant across every specification, including after adjusting for structural controls and for ego/alter degree ($\beta = 0.570$ to $0.577$, $p < 0.01$). A post hoc, wave-by-wave decomposition of the same model specification suggests this discrepancy is not primarily an artifact of pooling for statistical power: Wave 3, despite having as many or more No Religion egos and ties than every subsequent wave (25 egos, 166 ties), yields the smallest and least precise point estimate ($\beta = 0.15$, $p = 0.58$), while the coefficient rises across Waves 4 through 8 ($\beta = 0.50$ to $0.96$, generally $p < 0.06$) even as the number of unique No Religion egos shrinks to as few as 11. The observed in-group tie rate for No Religion students climbs from 10\% at Wave 3 to 29\% at Wave 8 against a roughly stable opportunity-adjusted baseline of 7--11\%, whereas Protestant and Other Religion students show no comparable trajectory---both are already significantly homophilous at Wave 3 (Protestant $p < 0.01$; Other Religion $p < 0.001$) and remain roughly flat across subsequent waves (Figure~\ref{fig:wave-by-wave-norelig}). This is suggestive of secular identity becoming a more salient basis for friendship selection only after the first year or two of college, but a formal test of a linear No Religion $\times$ wave interaction, with standard errors clustered on ego, is not statistically significant ($\beta = 0.093$, $p = 0.24$), so we treat this as a hypothesis for future work rather than a confirmed developmental trend. We therefore do not attempt to fully reconcile the Wave 3 and pooled results, but the wave-by-wave pattern indicates that Wave 3 is a genuinely distinctive cross-section for the No Religion group specifically, rather than merely an underpowered one. What is not in question, for any specification, is that adding each ego's activity level and each alter's popularity as controls leaves the Protestant and Other Religion coefficients essentially unchanged, indicating that their opportunity-adjusted homophily is not an artifact of differential nomination activity or popularity by religious group. Additionally, the negative effect of the same-dorm covariate suggests that within residential dorm units, the ``integrating forces'' of university life may pull students across religious lines more broadly.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{Plots/fig-wave-by-wave-norelig.png}
    \caption{Wave-Specific Opportunity-Adjusted Homophily Coefficients (with 95\% CIs), Model 2 Specification, for the Three Non-Catholic Groups. No Religion is statistically indistinguishable from the Catholic baseline at Wave 3, while Protestant and Other Religion are already significant; the No Religion coefficient is generally higher in later waves, though a formal linear trend test does not reach statistical significance (see Discussion).}
    \label{fig:wave-by-wave-norelig}
\end{figure}
