...
To trace these boundaries longitudinally, we fit a pooled model across Waves 3 to 8, comparing the entire friendship network to the subset of high-intensity, "Especially Close" friendships. Because individual students are observed repeatedly, we estimate robust standard errors clustered by the respondent's identifier.
...
To test whether minority homophily shifts over the college career, we fit a pooled interaction model between Centered Wave and Religious affiliation. However, in non-linear models like logistic regression, we cannot evaluate the interaction effect simply by interpreting the multiplicative coefficient or its $p$-value. To correctly interpret how the effect of religious affiliation evolves over time, we calculate and plot the marginal effects of each religious group relative to the Catholic baseline across all waves.

    \input{Tabs/tbl-interaction-reg.tex}

As visualized in Figure~\ref{fig:marginal-effects}, the marginal effects for the Protestant and Other Religion groups remain stable and highly parallel across Waves 3 through 8. The overlapping confidence intervals across time demonstrate that \textbf{the heightened homophily of Protestant and Other Religion groups does not significantly change over the course of college}. While the ``No Religion'' group shows a slight upward trajectory, their baseline propensity for homophily remains structurally suppressed.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{Plots/fig-marginal-effects.pdf}
    \caption{Average Marginal Effects of Religious Affiliation on Active Homophily over Time (Reference: Catholic).}
